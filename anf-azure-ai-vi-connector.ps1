# Azure NetApp Files - Azure AI Video Indexer Connector, created by Sean Luce, Azure NetApp Files
...
function Select-VideoIndexerAccount {
    ...
    $accountResponse = Invoke-AzRestMethod -SubscriptionId $subId -ResourceProviderName Microsoft.VideoIndexer -ResourceType "accounts" -ApiVersion 2024-01-01 -Method GET
    ...
    if (($videoIndexerAccounts.value).count -gt 0) {
        ...
        $videoIndexerResourceId = $videoIndexerAccounts.value[$accountSelected - 1].id
        ...
        Get-AzureVideoIndexerToken -subId $subId -accountName $accountName -videoIndexerResourceId $videoIndexerResourceId
    }
    else {
        write-host ""
...
function Get-AzureVideoIndexerToken {
    param(
        [string[]]$videoIndexerResourceId
    )
    $uri = 'https://management.azure.com' + $videoIndexerResourceId + '/generateAccessToken?api-version=2024-01-01'
    $payload = @{
...
function Send-Videos {
    ...
    $videoFiles | ForEach-Object {
        ...
        $uri = $baseUri + $videoIndexerLocation + '/Accounts/' + $Global:videoIndexerAccountId + '/Videos?accessToken=' + $accessToken + '&name=' + $prefix + $encodedFileName + '&privacy=private'
        $body = @{
...
function Find-KeywordVideoIndexer {
    ...
    foreach ($jsonFile in $jsonFiles) {
        ...
        foreach ($video in $videos) {
            ...
            foreach ($videoLabel in $videoLabels) {
                # | where-object {$_.name -eq $searchTerm}) {
...
function Show-MainMenu {
    ...
    while ($selection -ne 9) {
        ...
        write-host -foreground darkgray "  created by Sean Luce, Azure NetApp Files"
        write-host ""
